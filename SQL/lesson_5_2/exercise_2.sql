...
SELECT count(*) AS count_of_users, date_format(date(concat_ws('-',YEAR(now()),MONTH(birthday_at),DAY(birthday_at))), '%W') AS day_in_2019
FROM users
GROUP BY day_in_2019
